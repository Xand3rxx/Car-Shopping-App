...
function login()
{
    ...
    if($.trim(email).length > 0 && $.trim(password).length > 0)
    {
        var regex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
            ...
            {
                $.ajax({
                    url: "https://carshoppingapp.000webhostapp.com/cars/controllers/logUser.php",
                    method: "POST",
                    ...
                    success: function(response){
                        ...
                        if(response == "Succesful")
                        {
                              ...
                              alert("Your login was successful");
                               window.location.href = "https://carshoppingap.000webhostapp.com/cars/cart.php";
                        }
                        else
                        {
    ...
    if($.trim(email).length > 0 && $.trim(password).length > 0)
    {
        var regex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
            if(regex.test(email))
            {
                ...
                $.ajax({
                    url: "https://carshoppingapp.000webhostapp.com/cars/controllers/ad_lo.php",
                    method: "POST",
                    ...
                    success: function(data){
                        ...
                        if(data == "Successful")
                        {
                            ...
                            alert("Your login was successful");
                            window.location.href = "https://carshoppingapp.000webhostapp.com/cars/admin/dashboard.php";
                        }
                        else
                        {
                            alert('Invalid login credentials');
